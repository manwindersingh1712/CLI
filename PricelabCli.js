...
class PricelabCli {
	...
	filterListings = async results => {
		const listingData = results.listings.map(listing => {
			// finding out difference between begin and today's date
			const startDate = moment(listing.rateSummary.beginDate);
			const today = moment(Date.now());
			const differenceBetweenDates = moment(today, 'DD MM YYY').diff(
				moment(startDate, 'DD MM YYY'),
				'days'
			);

			// removing the rates for previous dates
			const rents = listing.rateSummary.rentNights
				? listing.rateSummary.rentNights.slice(differenceBetweenDates)
				: [];

			//  creating an object such as we have { date : price }
			let datesAndPrices = {};
			rents.map(rate => {
				datesAndPrices = {
					...datesAndPrices,
					[`${today.add(1, 'day').format('DD/MM/YY')}`]: rate
				};
			});
	...
	exportToCsv = async listingData => {
		// Generating the file name
		const dirPath = await path.resolve(__dirname, 'files');
		const files = await fs.readdir(dirPath);
		const fileName = `listings-${files.length + 1}.csv`;
		...
		XLSX.writeFile(workBook, path.join(dirPath, fileName));

		console.log(
			`Listings are exported successfully!!\nFile name: ${fileName}`
		);
	};

	makeApiCall = async config => {
		try {
